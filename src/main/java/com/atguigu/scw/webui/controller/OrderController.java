...
import com.atguigu.common.utils.AppDateUtils;
...
public class OrderController {
	...
	@PostMapping(value="/createOrder" , produces="text/html") //produces :给响应头设置content-type，告诉浏览器如何解析响应体内容
	public String createOrder(HttpSession session,OrderCreateInfoVo vo) {
		...
		if("00000".equals(orderResponse.getCode())) {
			//订单保存成功，调用支付宝的sdk的api，生成一个自动提交的带参数的 表单交给浏览器，然后浏览器会自动提交请求给支付宝服务端生成支付页面
			...
			alipayRequest.setReturnUrl(AlipayConfig.return_url);//立即响应 90%以上的准确率，不能作为最终的判断用户支付成功与否的条件
			alipayRequest.setNotifyUrl(AlipayConfig.notify_url);//异步延迟响应，最终判断用户是否支付成功的条件
	...
	public String toPayStep2(@RequestHeader("referer") String ref,Model model,Integer rtnCount,HttpSession session) {
		...
		//涉及到金额应该在java、代码中通过BigDecimel 处理
